...
if test -z "$AUTOGEN_CMAKE_ONLY"; then
  ...
  echo "To make a Debug build, run:"
  echo "autobuild build -c DebugFS_open -- --fmodstudio --compiler-cache -DCMAKE_CXX_COMPILER=/usr/bin/g++ -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON -DCWDS:BOOL=ON"
fi
